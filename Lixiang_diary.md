...
# 关于叠加渲染逻辑
理论公式<br>
 $C_i = \alpha_i c_i + (1 - \alpha_i)C_{i+1}$ 

这一层的颜色 + 后面所有层的折损颜色<br>
这需要知道“后面所有层”的结果，在渲染时这意味着需要“从后往前”处理<br>


代码里是这样的：<br>
C[ch] += features * alpha * T;<br>
这是“从前往后”的累加<br>
$$C_{total} = \sum_{i=1}^{N} c_i \cdot \alpha_i \cdot \prod_{j=1}^{i-1} (1-\alpha_j)$$

代码里的 $T$ 其实就是 $\prod_{j=1}^{i-1} (1-\alpha_j)$。<br>
    第一层：$T_1 = 1$ <br>
    第二层：$T_2 = (1-\alpha_1)$ <br>
    第三层：$T_3 = (1-\alpha_1)(1-\alpha_2)$ <br>


backward里写了很多的梯度，例如：高斯分布衰减指数G=e^power的梯度，对alpha的梯度，对三维均值的梯度，对旋转四元数的梯度，对缩放参数的梯度。
这些是用来链式法则求梯度的，不是最终要优化的59个参数
![总结的梯度依赖关系](image.png)

对于每一个高斯球，优化器（Adam）实际上是在同时维护以下几个不同的缓冲区：
```bash
参数类别                维度                作用
位置 (means)        3   决定空间坐标
旋转 (rotations)    4   决定椭球朝向 (四元数)
缩放 (scales)       3   决定椭球的胖瘦长短
透明度 (opacity)    1   决定球体浓淡
SH 系数 (shs)       3*(deg+1)^2    决定视角相关的颜色表现
```
